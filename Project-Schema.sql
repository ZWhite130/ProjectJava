...
CREATE OR REPLACE FUNCTION getPrice(requested_symbol VARCHAR2)
	RETURN FLOAT IS
	returnPrice FLOAT := 0;
	...
	BEGIN
		...
		WHERE (symbol = requested_symbol AND p_date = t_date );
...
CREATE OR REPLACE PROCEDURE allocateDeposit(login_name VARCHAR2, totalDeposit FLOAT)
	...
	BEGIN
		...
		LOOP
			...
			VALUES ( (SELECT MAX(trans_id) from TRXLOG ) + 1, login_name, prefers_row.symbol, transactionDate, 'buy', num_shares, getPrice(prefers_row.symbol), getPrice(prefers_row.symbol) * num_shares); 
			
		END LOOP;
		
		CLOSE prefers_cursor; --Don't forget to close trigger
...
BEGIN
	--Add the total amount of the deposit to our balance
	UPDATE CUSTOMER SET balance = balance + :NEW.amount
	WHERE CUSTOMER.login = :NEW.login;

	--Run allocateDeposit procedure
	--trigger onInsert_trxlog_buy will handle the modification of OWNS and the subtraction from balance
	SELECT balance INTO newBalance
	FROM CUSTOMER
	WHERE CUSTOMER.login = :NEW.login;
	
	allocateDeposit(:NEW.login, newBalance);
